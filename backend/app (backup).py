...
def getDatesBothIndices():
    ...
    cur.execute(f"""
        ...
        JOIN pacientes 
        ON pacientes.id=indice_cardiaco_table.pacientes_id
    ...
    cur.execute(f"""
        ...
        INNER JOIN pacientes ON pacientes.id=indice_pulmonar_table.pacientes_id
...
def getPatientIndiceByDates(id):
    ...
    if form["type"] in ["cardiaco", "both"]:
        cur.execute(f"""
            SELECT *, UNIX_TIMESTAMP(datetime)*1000 AS x, indice_cardiaco AS y FROM indice_cardiaco_table WHERE pacientes_id = "{id}" AND (datetime BETWEEN '{form["date"]} 00:00:00' AND '{form["final_date"]} 23:59:59')
        ...
        print(columns)
        for result in cardiaco:
            if result[1] in res.keys():
                res[result[1]]["cardiaco"].append(
                    dict(zip(columns, result[2:])))
                # res[result[0]]["cardiaco"][-1]["datetime"] = res[result[0]]["cardiaco"][-1]["datetime"].strftime("%d/%m/%Y %H:%M")
            else:
                res[result[1]] = {"cardiaco": [], "pulmonar": []}
                res[result[1]]["cardiaco"].append(
                    dict(zip(columns, result[2:])))
            res[result[1]]["cardiaco"][-1]["datetime"] = res[result[1]]["cardiaco"][-1]["datetime"].strftime("%d/%m/%Y %H:%M")
    # print(cardiaco)
    if form["type"] in ["pulmonar", "both"]:
        cur.execute(f"""
            SELECT *, UNIX_TIMESTAMP(datetime)*1000 AS x, indice_pulmonar AS y FROM indice_pulmonar_table WHERE pacientes_id = "{id}" AND (datetime BETWEEN '{form["date"]} 00:00:00' AND '{form["final_date"]} 23:59:59')
            ORDER BY datetime ASC""")
        pulmonar = cur.fetchall()
        columns = [x[0] for x in cur.description]
        columns = columns[2:]
        for result in pulmonar:
            if result[1] in res.keys():
                res[result[1]]["pulmonar"].append(
                    dict(zip(columns, result[2:])))
                # res[result[1]]["pulmonar"][-1]["datetime"] = res[result[1]]["pulmonar"][-1]["datetime"].strftime("%d/%m/%Y %H:%M")
            else:
                res[result[1]] = {"cardiaco": [], "pulmonar": []}
                res[result[1]]["pulmonar"].append(
                    dict(zip(columns, result[2:])))
